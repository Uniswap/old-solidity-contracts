...
def test_factory(t, uni_token, swap_token, uniswap_factory, exchange_abi, contract_tester, assert_tx_failed):
    ...
    assert u.remove_0x_head(uni_token_exchange.factoryAddress()) == uniswap_factory.address.hex()
    ...
    assert swap_token_exchange.ethFees() == 0
    assert swap_token_exchange.tokenFees() == 0
    assert swap_token_exchange.totalShares() == 0
    assert u.remove_0x_head(swap_token_exchange.tokenAddress()) == swap_token.address.hex()
    assert u.remove_0x_head(swap_token_exchange.factoryAddress()) == uniswap_factory.address.hex()
