...
def test_liquidity_investment_divestment(t, uni_token, uniswap_exchange, contract_tester, assert_tx_failed):
    t.s.mine()
    ...
    assert uniswap_exchange.ethPool() == 20*ETH
    assert uniswap_exchange.tokenPool() == 40*TOKEN
    ...
    assert uniswap_exchange.ethPool() == 15*ETH
    assert uniswap_exchange.tokenPool() == 30*TOKEN
    assert uniswap_exchange.getShares(t.a1) == 1000
    ...
    assert uniswap_exchange.ethPool() == 10*ETH
    assert uniswap_exchange.tokenPool() == 20*TOKEN
...
def test_public_fee_payout(t, uni_token, uniswap_exchange, contract_tester, assert_tx_failed):
    ...
    assert uniswap_exchange.ethPool() == new_market_eth + fee
    assert t.s.head_state.get_balance(uniswap_exchange.address) == new_market_eth + fee
...
def test_fee_invest_payout(t, uni_token, uniswap_exchange, contract_tester, assert_tx_failed):
    ...
    uniswap_exchange.investLiquidity(1, value=15*ETH, sender=t.k2)
    assert uniswap_exchange.ethFees() == 0
    assert uniswap_exchange.ethPool() == new_market_eth + 15*ETH + fee
...
def test_fee_divest_payout(t, uni_token, uniswap_exchange, contract_tester, assert_tx_failed):
    ...
    assert uniswap_exchange.ethFees() == 0
    assert uniswap_exchange.ethPool() == (new_market_eth + fee)/2
